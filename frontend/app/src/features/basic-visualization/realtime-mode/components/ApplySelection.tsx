...
export function ApplySelection() {
  const { deviceId, modelId, dataType } = useDeviceModelIdContext();
  ...
  const { setImageSrc, setInferenceData } = useImageInferenceContext();
  ...
  function setInitialState(deviceId: string): void {
    ...
    Promise.all([
      ...
      .catch((err) => {
        console.error(err);
        alert("Error fetching image or configuration.");
