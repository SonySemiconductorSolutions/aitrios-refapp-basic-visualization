...
function getNewParametersV2(config: ConfigurationV2, dataType: string): any {
  if (dataType == "ObjectDetection") {
    return config.edge_app.custom_settings?.ai_models?.detection?.parameters;
  } else if (dataType == "Classification") {
    return config.edge_app.custom_settings?.ai_models?.classification
      ?.parameters;
  }
}
...
export default function ExtraSettingsPanel() {
  ...
  const { modelId, setModelId, modelIdsOfDevice, dataType } =
  ...
  const updatePPLParametersInConfigV2 = (
    ...
    const newParameters = getNewParametersV2(newConfig, dataType);
    ...
    if (newConfig.edge_app.custom_settings) {
      if (!prevConfiguration.edge_app.custom_settings) {
        ...
        if (dataType == "ObjectDetection") {
          ...
          if (
            prevConfiguration.edge_app.custom_settings.ai_models.classification
              .ai_model_bundle_id !=
            newConfig.edge_app.custom_settings.ai_models?.classification
              .ai_model_bundle_id
          ) {
            setModelId(
              newConfig.edge_app.custom_settings.ai_models.classification
