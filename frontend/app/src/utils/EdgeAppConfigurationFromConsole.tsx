...
import { StatusResponse, DataType } from "../types/types";
import {
  defaultInputHeight,
  defaultInputWidth,
  ...
  defaultMaxPredictions,
  defaultDnnOutputClasses,
...
export function getDefaultConfiguration(
  ...
  if (dataType == "ObjectDetection") {
    ...
    parameters = {
      Mode: 1,
      PPLParameter: {
        dnn_output_classes: defaultDnnOutputClasses,
        max_predictions: defaultMaxPredictions,
      },
    };
  }
  const command: Command = {
...
export function getDefaultConfigurationV2(
  modelId: string,
  dataType: DataType,
): CustomSettingsData {
  if (dataType == "ObjectDetection") {
...
function fillInMissingValuesWithDefault(
  ...
  const defaultCustomSettings: CustomSettingsData = getDefaultConfigurationV2(
    modelId,
    dataType,
  );
  ...
  if (!configuration.edge_app.custom_settings?.ai_models) {
    ...
    if (dataType == "ObjectDetection") {
      ...
      if (
        !configuration.edge_app.custom_settings.ai_models ||
        !configuration.edge_app.custom_settings.ai_models.classification
      ) {
        configuration.edge_app.custom_settings.ai_models =
          defaultCustomSettings.ai_models;
        isUpdated = true;
        ...
        configuration.edge_app.custom_settings.ai_models.classification.parameters =
          defaultCustomSettings.ai_models.classification.parameters;
        isUpdated = true;
      }
    }
  }
  ...
  if (consoleType == "ONLINE V2") {
    ...
    if (initialConfiguration.edge_app.custom_settings) {
      if (dataType == "ObjectDetection") {
        ...
        initialConfiguration.edge_app.custom_settings.ai_models.classification.ai_model_bundle_id =
          modelId.substring(6, 12);
      }
    }
    if (requiresUpdate) {
    ...
    let initialConfiguration: ConfigurationV1 = getDefaultConfiguration(
      file_name,
      dataType,
    );
