...
class ClientInferface(ABC, Generic[Conf]):
    ...
    def get_images(
        ...
        """Obtain a list of images stored in the image directory.

        Args:
            device_id (str): Device ID
            sub_directory_name (str): Image Directory

        Returns:
            list: List of dictionary containing image timestamp and image data.
        """

    @abstractmethod
    def get_inferences_associated_with_images(
        self, device_id: str, image_list: list[ImageAndInference]
    ) -> list[ImageAndInference]:
        """Obtain inference results tied to the timestamp of the image.

        Args:
            device_id (str): Device ID
            image_list (list[dict[str, str]]): List of image data

        Returns:
            list: List of dictionary containing image, timestamp, model_id and inference.
        """
