...
class OnlineConsoleClientV1(ClientInferface[ConfigurationV1]):
    ...
    def get_latest_data(
        ...
        try:
            ...
            inference = {"timestamp": None, "content": None, "model_id": None}
    ...
    def get_images(
        ...
        logger.info(f"Fetching uploaded images for device ID '{device_id}'.")

        try:
            insight_api = InsightApi(self.get_client())

            image_list: list[ImageAndInference] = []
            continuation_time: str | None = None
            limit = 256  # Value range: 1 to 256
            ...
            start_datetime = self._convert_ts_to_datetime(
                response.images[0].name.replace(".jpg", "")
            )
            ...
            tenhours_later_datetime = start_datetime + datetime.timedelta(hours=10)
            current_datetime = datetime.datetime.now(datetime.timezone.utc)
            end_datetime = (
                current_datetime
                if current_datetime < tenhours_later_datetime
                else tenhours_later_datetime
            )
            continuation_time = end_datetime.strftime("%Y%m%d%H%M")
            ...
            while True:
                response = insight_api.get_images(
                    ...
                    to_datetime=continuation_time,
                )

                if len(response.images) > 0:
                    for data in response.images:
                        timestamp = data.name.replace(".jpg", "")
                        image_content = data.contents
                ...
                if len(response.images) >= limit:
                    rearend_datetime = self._convert_ts_to_datetime(
                        response.images[-1].name.replace(".jpg", "")
                    )
                    continuation_time = rearend_datetime.strftime("%Y%m%d%H%M")
                else:
                    break

            logger.info(
                f"Successfully retrieved image data for device ID '{device_id}'"
            )
            return image_list
        except ApiException as api_error:
            logger.error(
                f"API error while retrieving image ata of device id {device_id}: {api_error}",
                exc_info=True,
            )
            raise Exception(
                f"API error while retrieving omage data of device id {device_id}: {api_error}"
            )
    ...
    def get_inferences_associated_with_images(
        ...
        try:
            ...
            for data in image_list:
                ...
                inference = insight_api.get_inference_results(
                    device_id=device_id,
                    number_of_inferenceresults=1,
                    raw=1,
                    time=timestamp,
                ...
                if len(inference) != 0:
                    inference_content = inference[0].inference_result.inferences[0].o
                    model_id = inference[0].model_id

                    data["inference"] = inference_content
            ...
            logger.error(
                f"API error while retrieving inference data of device id {device_id}: {api_error}",
                exc_info=True,
            )
            raise Exception(
                f"API error while retrieving inference data of device id {device_id}: {api_error}"
    ...
    def _convert_ts_to_datetime(self, timestamp: str) -> datetime.datetime:
