...
class OnlineConsoleClientV2(ClientInferface):
        ...
        try:
            ...
            if response.inferences is not None and len(response.inferences) > 0:
                ...
                inference = {
                    ...
                    "model_id": response.inferences[0].model_id,
                }
    ...
    def get_images(
        ...
        try:
            ...
            while True:
                ...
                if (
                    response.continuation_token is not None
                    and response.continuation_token != ""
                ):
            ...
            logger.error(
                f"API error while retrieving image data of device id {device_id}: {api_error}",
                exc_info=True,
            )
            raise Exception(
                f"API error while retrieving image data of device id {device_id}: {api_error}"
    ...
    def get_inferences_associated_with_images(
        ...
        try:
            ...
            from_datetime = self._convert_ts_to_datetime(image_list[-1]["timestamp"])
            to_datetime = self._convert_ts_to_datetime(image_list[0]["timestamp"])

            response = insight_api.inferenceresults_get(
                devices=[device_id],
                from_datetime=from_datetime.isoformat().replace("+00:00", ""),
                to_datetime=to_datetime.isoformat().replace("+00:00", ""),
            )

            inference_dict = {}
            if response.inferences is not None:
                for inference_item in response.inferences:
                    inference = inference_item.inferences[0].o
                    timestamp_iso = inference_item.inferences[0].t

                    numeric_timestamp = datetime.datetime.fromisoformat(
                        timestamp_iso
                    ).strftime("%Y%m%d%H%M%S%f")[:-3]

                    inference_dict[numeric_timestamp] = {
                        "inference": inference,
                        "model_id": inference_item.model_id,
                    }

            for item in image_list:
                if item["timestamp"] in inference_dict:
                    item["inference"] = inference_dict[item["timestamp"]]["inference"]
                    item["model_id"] = inference_dict[item["timestamp"]]["model_id"]
